...
close all;
%--------Inputs-------------------------------------------------
N=6000; %Number of data samples to send across the Rician Channel
...
%--------------------------------------------------------------
dataSym=(rand(1,N)>0.5)'; %data generation
M=16;
hMod = comm.RectangularQAMModulator('ModulationOrder',M,'BitInput',true);
hDemod = comm.RectangularQAMDemodulator('ModulationOrder',M,'BitOutput',false);
x = step(hMod,dataSym);%QAM16 modulation..
...
% rxSym = qamdemod(r_ricean,16,0,'gray');  % Demodulate signal
dataOut = step(hDemod,y_ricean_cap);%QAM16 demodulation
simBER_ricean(i) = biterr(d,dataOut);
disp(K);
end
simBER_ricean=simBER_ricean/N;
%Simulated BER;
EbN0=10.^(EbN0dB/10); %Eb/N0 in Linear Scale
semilogy(EbN0dB,simBER_ricean,'LineWidth',2);hold on
end
axis([0 20 10^-5 10^0]);
