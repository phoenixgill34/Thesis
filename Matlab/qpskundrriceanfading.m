...
hold on;
semilogy(EbN0dB,symErrSimulatedqpsk(2,:),'--','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqpsk(3,:),'-o','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqpsk(4,:),'-p','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqpsk(5,:),'-.s','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqpsk(6,:),'-','LineWidth',2);
xlabel('E_b/N_0(dB)');
ylabel('BER');
title(['BER For QPSK Under Rician Fading Environment']);
grid on;
...
lgd.FontSize=20;
axis([0 20 1e-6 1]);
fprintf('Total Time For execution is %f mins\n',toc(totaltime)/60);