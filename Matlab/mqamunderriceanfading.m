...
for count=1:length(Kf)
    ...
    symErrSimulatedqpsk(count,index) = numerr/numBits;
    biterrsim(count,index) = symErrSimulatedqpsk(count,index)/k;
    index=index+1;
    end
end
%% 16 QAM
...
for count=1:length(Kf)
    ...
    symErrSimulatedqam(count,index) = numerr/numBits;
    biterrsim(count,index) = symErrSimulatedqam(count,index)/k;
    index=index+1;
...
%%
figure(1);
semilogy(EbN0dB,symErrSimulatedqpsk(1,:),'-d','LineWidth',2);
hold on;
semilogy(EbN0dB,symErrSimulatedqam(1,:),'--','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqam64(1,:),'-o','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqpsk(2,:),'-p','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqam(2,:),'-.s','LineWidth',2);
semilogy(EbN0dB,symErrSimulatedqam64(2,:),'-','LineWidth',2);
xlabel('E_b/N_0(dB)');
ylabel('BER');
title(['BER For OFDM Under Rician Fading Environment']);
grid on;
set(gca,'fontsize',30,'box','on','LineWidth',2,'GridLineStyle','--','GridAlpha',0.7);
lgd = legend('QPSK K = 22.6 dB','QPSK K = -0.9 dB','16QAM K = 22.6 dB','16QAM K = -0.9 dB',...
    '64QAM K = 22.6 dB','64QAM K = -0.9 dB');
lgd.FontSize=20;
axis([0 20 1e-6 1]);